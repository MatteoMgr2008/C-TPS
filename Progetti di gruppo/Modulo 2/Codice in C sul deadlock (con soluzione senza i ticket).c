...
void *cliente1(void *arg) {
    ...
    // Risoluzione del problema del deadlock: se la risorsa "stazione di riparazione e analisi" è occupata da un thread (come per esempio da "cliente1" o da "cliente2"), allora il codice forza il rilascio della risorsa "stazione di riparazione e analisi"
    if(flag_stazione_riparazione_analisi==true){ // Controlla se la risorsa "stazione di riparazione e analisi" è occupata
        sem_post(&sem_stazione_riparazione_analisi); // Rilascio forzato della risorsa "stazione di riparazione e analisi" per evitare il deadlock
...
void *cliente2(void *arg) {
    ...
    // Il thread denominato "cliente2" alloca subito la risorsa "stazione di riparazione e analisi"
    sem_wait(&sem_stazione_riparazione_analisi);
    flag_stazione_riparazione_analisi=true; // La risorsa "stazione di riparazione e analisi" è attualmente occupata
